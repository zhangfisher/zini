...
pub fn main() !void {
    ...
    // 示例 1: 基本 Schema 查询
    {
        ...
        std.debug.print("port -> 类型: {s}\n", .{port_schema.datatype.typeName()});
        std.debug.print("timeout -> 类型: {s}\n", .{timeout_schema.datatype.typeName()});
        std.debug.print("enabled -> 类型: {s}\n", .{enabled_schema.datatype.typeName()});
        std.debug.print("name -> 类型: {s}\n", .{name_schema.datatype.typeName()});
